...
public ref class ExtractionOptions {
	...
	/// convert, or in a memory buffer in InputBytes, or through
	/// InputStream. InputText and InputFields are assumed to contain
	/// nothing but plain text (not HTML or RTF, which must be
	/// provided through InputFile or InputBytes).
	/// 
	...
	/// InputBytes or InputStream.
	/// 
	/// Even when InputBytes or InputStream is used, a filename
	/// should be provided in InputFile if possible to tell dtSearch
	/// the original filename extension, which can provide useful
