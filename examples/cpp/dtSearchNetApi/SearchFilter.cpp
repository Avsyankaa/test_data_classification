...
namespace dtSearch { namespace Engine
{
	...
	SearchFilter::SearchFilter (dtsSearchFilter *filter)
	{	CountCreate(otSearchFilter);
		if (filter)
	...
	long SearchFilter::getIndexSelections (long whichIndex) __gc []
	{
		...
		long *destArray = new long[arrLength];
		dtssIntArrayGetItems (arrHandle, destArray, arrLength);
		dtssIntArrayDelete (arrHandle);

		// Copy unmanaged array to managed array
		long managedArr __gc [] = new long __gc [arrLength];
		for (long i = 0; i < arrLength; i++)
		{
			managedArr [i] = destArray [i];
		}
		delete [] destArray;
	...
	void SearchFilter::DisposeUnmanagedItems() {
		debugLog("SearchFilter::DisposeUnmanagedItems...");
		if (m_searchFilter) {
			dtsSearchFilter *toDelete = m_searchFilter;
			m_searchFilter = 0;
			delete toDelete;
			}
		debugLog("SearchFilter::DisposeUnmanagedItems... Done");
		}
